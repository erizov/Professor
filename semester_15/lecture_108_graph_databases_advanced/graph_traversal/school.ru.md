# Граф Traversal

## Простое объяснение

Граф Traversal — это способ обойти все узлы графа, посетив каждый ровно один раз. 
Алгоритм систематически проходит по связям между узлами, исследуя структуру графа.

## Где применяется

- поиск пути в лабиринте или на карте;
- анализ социальных сетей (поиск друзей, рекомендации);
- поиск кратчайшего пути в навигационных системах;
- анализ связей между веб-страницами в поисковых системах;



## Пример

Обходим граф друзей в социальной сети:

Граф: Иван → [Мария, Пётр], Мария → [Анна], Пётр → [Анна]

BFS (в ширину):
Шаг 1: Начинаем с "Иван" (уровень 0)
Шаг 2: Посещаем всех друзей "Иван": "Мария", "Пётр" (уровень 1)
Шаг 3: Посещаем друзей "Мария" и "Пётр": "Анна" (уровень 2)

Порядок обхода: Иван → Мария, Пётр → Анна



## Вопросы для самопроверки

### Базовые

1. Что делает алгоритм граф traversal?
2. В каких ситуациях используется граф traversal?
3. Какие данные нужны для работы алгоритма?

### Средние

4. Как граф traversal обрабатывает граничные случаи?
5. Какие преимущества и недостатки у граф traversal?
6. Можно ли улучшить работу граф traversal?

### Сложные

7. Какова временная сложность граф traversal?
8. Как граф traversal работает с большими объёмами данных?
9. Как можно оптимизировать граф traversal?



## Практические задания

### Уровень 1 (Лёгкий)

Выполни простую операцию с алгоритмом граф traversal. 
Используй небольшой набор данных (3-5 элементов) и выведи результат.

### Уровень 2 (Средний)

Примени алгоритм граф traversal к более сложному набору данных. 
Проанализируй результат, объясни каждый шаг работы алгоритма.

### Уровень 3 (Продвинутый)

Напиши реализацию алгоритма граф traversal на языке программирования. 
Добавь обработку ошибок, проверку входных данных, тесты и документацию.

