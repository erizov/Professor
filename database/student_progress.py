...
def create_progress_schema(cursor):
    ...
    # Indexes
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_progress_student ON algorithm_progress(student_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_progress_algorithm ON algorithm_progress(algorithm_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_tests_student ON test_results(student_id)')
...
class StudentProgressTracker:
    ...
    def start_algorithm(self, algorithm_id: int):
        ...
        ''', (self.student_id, algorithm_id, datetime.now().isoformat(),
    ...
    def complete_algorithm(self, algorithm_id: int, time_spent_minutes: int = 0):
        ...
        ''', (datetime.now().isoformat(), time_spent_minutes,
              datetime.now().isoformat(), self.student_id, algorithm_id))
        self.conn.commit()
        
        # Check for achievements
        self._check_achievements()
    
    def record_test_result(self, algorithm_id: int, test_score: float,
