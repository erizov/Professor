...
class AlgorithmTestCase(unittest.TestCase):
    ...
    def assert_sorted(self, result: List[Any], 
        ...
        if original:
            self.assertEqual(
                ...
                "Result length doesn't match original"
            )
            self.assertEqual(
                sorted(original), result,
                "Result doesn't match expected sorted array"
            )
    
    def assert_search_result(self, result: int, 
                            target: Any, 
                            array: List[Any],
                            found: bool = True):
    ...
    def assert_performance(self, func: Callable, 
...
def create_test_suite(algorithm_name: str, 
                     test_cases: List[dict]) -> unittest.TestSuite:
