# Хеш-таблица

## Простое объяснение

Хеш-таблица — это способ быстро находить данные по ключу. Алгоритм преобразует ключ 
в число (хеш-код), которое указывает, где хранится нужная информация.

## Где применяется

- быстрый поиск контактов в телефонной книге по имени;
- хранение паролей в безопасном виде (хеширование);
- индексация страниц в поисковых системах;
- кэширование данных для ускорения работы приложений;



## Пример

Храним контакты в телефонной книге:

Имя: "Иван" → хеш-функция → индекс 5 → храним номер "123-456"
Имя: "Мария" → хеш-функция → индекс 2 → храним номер "789-012"
Имя: "Пётр" → хеш-функция → индекс 5 → коллизия! Используем цепочку

Поиск "Иван":
Шаг 1: Хеш "Иван" → индекс 5
Шаг 2: Проверяем элемент по индексу 5 → найдено "Иван" с номером "123-456"

Поиск очень быстрый — O(1) в среднем случае!



## Вопросы для самопроверки

### Базовые

1. Что делает алгоритм хеш-таблица?
2. В каких ситуациях используется хеш-таблица?
3. Какие данные нужны для работы алгоритма?

### Средние

4. Как хеш-таблица обрабатывает граничные случаи?
5. Какие преимущества и недостатки у хеш-таблица?
6. Можно ли улучшить работу хеш-таблица?

### Сложные

7. Какова временная сложность хеш-таблица?
8. Как хеш-таблица работает с большими объёмами данных?
9. Как можно оптимизировать хеш-таблица?



## Практические задания

### Уровень 1 (Лёгкий)

Выполни простую операцию с алгоритмом хеш-таблица. 
Используй небольшой набор данных (3-5 элементов) и выведи результат.

### Уровень 2 (Средний)

Примени алгоритм хеш-таблица к более сложному набору данных. 
Проанализируй результат, объясни каждый шаг работы алгоритма.

### Уровень 3 (Продвинутый)

Напиши реализацию алгоритма хеш-таблица на языке программирования. 
Добавь обработку ошибок, проверку входных данных, тесты и документацию.

