...
class GraphVisualizer:
    ...
    def visualize_dijkstra(
        ...
        while pq:
            ...
            visited.add(current)
            self.record_step(
                visited, current, [n for _, n in pq], path.get(current, [])
            )
            ...
            for neighbor in self.graph.get(current, []):
                if neighbor not in visited:
                    ...
                    if new_dist < distances[neighbor]:
                        ...
                        self.record_step(
                            visited, current, [n for _, n in pq], path.get(neighbor, [])
                        )
    ...
    def _create_animation(self) -> animation.FuncAnimation:
        ...
        anim = animation.FuncAnimation(
            fig, animate, frames=len(self.steps), interval=500, repeat=False, blit=False
        )
