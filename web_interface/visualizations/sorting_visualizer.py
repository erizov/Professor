...
class SortingVisualizer:
    ...
    def _create_animation(self) -> animation.FuncAnimation:
        ...
        ax.set_title(f'{self.algorithm_name} - Comparisons: {self.comparisons}, Swaps: {self.swaps}')
        ax.set_xlabel('Index')
        ax.set_ylabel('Value')
        ...
        def animate(frame):
            if frame < len(self.steps):
                ...
                ax.set_title(f'{self.algorithm_name} - Step {frame + 1}/{len(self.steps)}')
...
def visualize_sorting_algorithm(algorithm_name: str, data: List[int], output_file: str = None):
    """
