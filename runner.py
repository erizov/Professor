...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run algorithm implementations"
    )
    parser.add_argument(
        "--semester",
        type=int,
        required=True,
        help="Semester number (1-4)"
    )
    parser.add_argument(
        "--lecture",
        required=True,
        help="Lecture number (e.g., '01', '02')"
    )
    parser.add_argument(
        "--algorithm",
        required=True,
    ...
    # Build path
    algorithm_path = Path(
        f"semester_{args.semester}/"
        f"lecture_{args.lecture}/"
        f"{args.algorithm}"
    )
    
    if not algorithm_path.exists():
        print(
            f"Error: Algorithm path {algorithm_path} not found",
            file=sys.stderr
        )
        return 1
