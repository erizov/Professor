...
def python_to_java_class(py_impl: str, algorithm_name: str) -> str:
    ...
    if init_match:
        ...
        for match in re.finditer(attr_pattern, init_body):
            ...
            if attr_type == 'Dict':
                fields.append(f'    private Map<String, Object> {attr_name} = new HashMap<>();')
            elif attr_type == 'List':
                fields.append(f'    private List<Object> {attr_name} = new ArrayList<>();')
            else:
                fields.append(f'    private Map<String, Object> {attr_name} = new HashMap<>();')
        ...
        for attr in simple_attrs:
            if not any(attr in f for f in fields):
                fields.append(f'    private Map<String, Object> {attr} = new HashMap<>();')
    ...
    for method_name, method_body in methods:
        ...
        method_def_match = re.search(r'def\s+' + re.escape(method_name) + r'\s*\(([^)]*)\)', py_impl)
        params = []
        if method_def_match:
        ...
        if params:
            param_list = ', '.join([f'String {p}' if 'id' in p or 'user' in p or 'recipient' in p 
                                   else f'Object {p}' for p in params])
            ...
            if 'f"' in method_body:
                ...
                if str_match:
                    ...
                    if '{int(time.time())}' in template:
                        ...
                        if len(parts) > 1:
                            java_str = parts[0] + '" + ' + parts[1].split('}')[0] + ' + "' + parts[1].split('}')[1] if '}' in parts[1] else parts[1]
        ...
        # Boolean return
        elif '-> bool' in method_body or 'return True' in method_body or 'return False' in method_body:
        ...
        # Dict/Map return
        elif '-> Dict' in method_body or 'dict(' in method_body or ("{" in method_body and "'" in method_body):
...
def is_generic_or_todo(java_content: str) -> bool:
    ...
    if 'return null;' in java_content or 'return data;' in java_content:
        ...
        code_lines = [l.strip() for l in java_content.split('\n') 
        ...
        has_logic = any(keyword in java_content for keyword in [
            'if (', 'for (', 'while (', 'switch (', 
            'result.put', 'result.add', 'list.add',
            'map.put', 'set.add', '=', 'return "',
