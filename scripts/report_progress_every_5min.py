...
# -*- coding: utf-8 -*-
"""
Report enhancement progress every 5 minutes.
Run this script to monitor the enhancement process.
"""
...
def print_progress_report(stats: Dict, start_time: datetime):
    ...
    if stats["complete"]:
        ...
        if stats["total_processed"] > 0:
            ...
            print("Status: Starting up or waiting for first enhancement...")
        
        if stats["last_update"]:
            time_since_update = datetime.now() - stats["last_update"]
            if time_since_update.total_seconds() < 60:
                print(f"🔄 Last update: {time_since_update.seconds} seconds ago")
            else:
                print(
                    f"🔄 Last update: {int(time_since_update.total_seconds() / 60)} "
                    f"minutes ago"
                )
