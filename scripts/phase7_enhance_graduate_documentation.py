...
def main():
    ...
    for i, readme_path in enumerate(readme_files, 1):
        ...
        if (short_desc_updated + intro_updated + detail_updated) % 50 == 0 and (
            short_desc_updated + intro_updated + detail_updated
        ) > 0:
            print(
                f"[PROGRESS] Processed {i}/{len(readme_files)} files, updated {short_desc_updated + intro_updated + detail_updated}..."
            )
    ...
    print(
        f"Total enhancements: {short_desc_updated + intro_updated + detail_updated} files"
    )
