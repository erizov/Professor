...
def determine_algorithm_category(algorithm_name: str, lecture_name: str) -> Optional[str]:
    """Determine algorithm category from name and lecture."""
    algo_lower = algorithm_name.lower()
    lecture_lower = lecture_name.lower()
    
    if any(word in algo_lower or word in lecture_lower for word in ['sort', 'bubble', 'selection', 'insertion', 'heap', 'counting', 'radix', 'bucket', 'merge', 'quick']):
        return 'sorting'
    elif any(word in algo_lower or word in lecture_lower for word in ['search', 'linear', 'binary', 'jump', 'interpolation']):
        return 'searching'
    elif any(word in algo_lower or word in lecture_lower for word in ['tree', 'bst', 'avl', 'trie', 'binary_tree']):
        return 'tree'
    elif any(word in algo_lower or word in lecture_lower for word in ['graph', 'bfs', 'dfs', 'dijkstra', 'bellman', 'floyd']):
        return 'graph'
    elif any(word in algo_lower or word in lecture_lower for word in ['hash', 'hash_table', 'hash_map']):
        return 'hash'
    elif any(word in algo_lower or word in lecture_lower for word in ['dynamic', 'fibonacci', 'knapsack', 'edit_distance', 'longest']):
...
def has_framework_examples(content: str) -> bool:
    ...
    has_java = '```java' in content and any(fw in content for fw in ['Spring', 'Java', 'Arrays', 'Collections'])
...
def main():
    ...
    for i, readme_path in enumerate(readme_files, 1):
        ...
        lecture_name = readme_path.parent.parent.name if readme_path.parent.parent else ""
        
        if add_framework_examples_to_readme(readme_path, algorithm_name, lecture_name):
            updated += 1
            if updated % 50 == 0:
                print(f"[PROGRESS] Processed {i}/{len(readme_files)} files, updated {updated}...")
