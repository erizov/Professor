        ...
        if semester_readme.exists():
            with open(semester_readme, "r", encoding="utf-8") as f:
                content_parts.append(
                    (f"## {semester_dir.name.replace('_', ' ').title()}\n\n", f.read())
                )
...
"""
    ...
    # First, try to extract from synced textbook content
    full_content += """
\\newpage
...
"""
    ...
    # Read textbook to extract synced amendment content
    textbook_path = base_path / "COMPREHENSIVE_COURSE_TEXTBOOK.md"
    amendment_content_map = {}
    
    if textbook_path.exists():
        with open(textbook_path, "r", encoding="utf-8") as f:
            textbook_content = f.read()
        
        # Extract synced sections from textbook
        import re
        amendment_files = [
        ...
        for doc_file in amendment_files:
            start_marker = f"<!-- SYNC_START:{doc_file} -->"
            end_marker = f"<!-- SYNC_END:{doc_file} -->"
            
            start_pos = textbook_content.find(start_marker)
            if start_pos != -1:
                end_pos = textbook_content.find(end_marker, start_pos)
                if end_pos != -1:
                    # Extract content between markers
                    section_content = textbook_content[start_pos + len(start_marker):end_pos].strip()
                    # Remove the note line
                    lines = section_content.split("\n")
                    content_lines = []
                    for line in lines:
                        if "*This section is synchronized" not in line:
                            content_lines.append(line)
                    amendment_content_map[doc_file] = "\n".join(content_lines).strip()
    
    # Add supporting documents (use synced content if available, otherwise read from files)
    supporting_docs = [
    ...
    for doc_file, doc_title in supporting_docs:
        ...
        if doc_file in amendment_content_map:
            doc_content = amendment_content_map[doc_file]
            full_content += f"\n\n## Appendix: {doc_title}\n\n"
            full_content += f"*This content is synchronized with [{doc_file}](../{doc_file}).*\n\n"
