...
def determine_category(algorithm_name: str, lecture_name: str, content: str) -> str:
    ...
    if any(
        s in algo_lower
        for s in ["sort", "bubble", "selection", "insertion", "merge", "quick", "heap"]
    ):
        return "sorting"
        ...
        s in algo_lower or s in lecture_lower for s in ["sql", "database", "query"]
    ):
        return "database"
    return "general"
...
def replace_todo_implementation_fixed(
    ...
    try:
        ...
        if match:
            ...
            if impl_func_match:
                ...
                original_func_match = re.search(
                    r"def\s+(\w+)\(", content[func_start : func_start + 200]
                )
                if original_func_match:
                    original_func_name = original_func_match.group(1)
                    # Use original function name
                    impl_body = re.sub(
                        r"def\s+\w+\(", f"def {original_func_name}(", impl, count=1
                    )
                    impl_body = (
...
def find_algorithms_needing_work() -> (
    ...
    for algo_file in ROOT.rglob("**/algorithm.py"):
        ...
        try:
            ...
            if (
                "TODO" in content
                or ("pass" in content and "def " in content)
                or (
                    "return None" in content
                    and content.count("return") == 1
                    and "def " in content
                )
            ):
...
def main():
    ...
    for i, (algo_file, algo_name, category) in enumerate(need_impl, 1):
        if replace_todo_implementation_fixed(algo_file, algo_name, category):
            ...
            if total_completed % 20 == 0:
                ...
                commit_changes(
                    f"Complete algorithms: {total_completed} completions ({impl_completed} impl, {framework_completed} frameworks)"
                )
    ...
    if total_completed > 0:
        commit_changes(
            f"Complete all algorithms FINAL: {total_completed} total ({impl_completed} impl, {framework_completed} frameworks)"
        )
