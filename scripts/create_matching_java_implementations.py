...
def convert_python_class_to_java(py_content: str, algorithm_name: str) -> str:
    ...
    if init_match:
        ...
        attr_patterns = [
            (
                r"self\.(\w+)\s*:\s*Dict\[str,\s*dict\]",
                "Map<String, Map<String, Object>>",
            ),
            (
                r"self\.(\w+)\s*:\s*Dict\[str,\s*List\[str\]\]",
    ...
    for match in re.finditer(method_pattern, py_content):
        ...
        for param in params_str.split(","):
            ...
            if param and param != "self":
                ...
                param_type = (
                    "String"
                    if "str" in param or "id" in param_name or "user" in param_name
                    else "Object"
                )
                if "List[" in param:
        ...
        param_calls = ", ".join(
            [
                (
                    '""'
                    if ptype == "String"
                    else "new ArrayList<>()" if "List" in ptype else "null"
                )
...
def convert_python_method_body_to_java(py_body: str, field_map: Dict[str, str]) -> str:
    ...
    for attr, key, value in dict_assigns:
        if attr in field_map:
            java_lines.append(
                f"        Map<String, Object> {attr}_entry = new HashMap<>();"
            )
            # Extract dict keys
    ...
    if "not in" in py_body:
        ...
        if in_match:
            ...
            if attr in field_map:
                java_lines.append(
                    f"        if (!{field_map[attr]}.containsKey({var})) {{"
                )
                java_lines.append(
                    f"            {field_map[attr]}.put({var}, new ArrayList<>());"
                )
                java_lines.append("        }")
    ...
    if ".append(" in py_body:
        ...
        if append_match:
            ...
            if attr in field_map:
                java_lines.append(
                    f"        ((List<String>){field_map[attr]}.get({key})).add({value});"
                )
    ...
    if "-> str" in py_body:
        ...
        if "return user in" in py_body or "in self." in py_body:
            in_match = re.search(
                r"return\s+(\w+)\s+in\s+self\.(\w+)\[([^\]]+)\]", py_body
            )
            if in_match:
                var = in_match.group(1)
                attr = in_match.group(2)
                ...
                if attr in field_map:
                    java_lines.append(
                        f"        if ({field_map[attr]}.containsKey({key})) {{"
                    )
                    java_lines.append(
                        f"            return ((List<String>){field_map[attr]}.get({key})).contains({var});"
                    )
                    java_lines.append("        }")
...
def create_java_from_python(algorithm_folder: Path, algorithm_name: str) -> bool:
    ...
    if java_file.exists():
        ...
        if (
            "// TODO" not in java_content
            and "TODO:" not in java_content
            and "return null;" not in java_content
            and "return data;" not in java_content
        ):
