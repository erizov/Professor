...
def infer_category(lecture_path: str) -> Optional[str]:
    ...
    if any(x in path_lower for x in ["sorting", "sort"]):
        ...
        return "monitoring"
    elif any(x in path_lower for x in ["ml", "machine_learning", "neural", "cnn", "rnn", "llm", "transfer_learning"]):
        return "ml"
...
def update_short_description(readme_path: Path, algorithm_name: str, lecture_path: str) -> bool:
            ...
            if re.search(pattern2, content, re.IGNORECASE | re.DOTALL):
                ...
                for i, line in enumerate(lines):
                    if "### Short Description" in line:
                        ...
                        if "algorithm that solves a specific computational problem efficiently" in line:
                            continue
                        elif "An algorithm" in line and "solves" in line and "efficiently" in line:
                            continue
                        else:
                            new_lines.append(line)
    ...
    for semester_dir in ROOT.glob("semester_*"):
        ...
        for lecture_dir in semester_dir.iterdir():
            ...
            for algo_dir in lecture_dir.iterdir():
                ...
                if update_short_description(readme_path, algorithm_name, str(lecture_dir)):
                    updated_count += 1
