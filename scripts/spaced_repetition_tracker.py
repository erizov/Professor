...
class SpacedRepetitionTracker:
    ...
    def _load_data(self) -> Dict:
        ...
        if TRACKER_FILE.exists():
            with open(TRACKER_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
        return {"algorithms": {}, "last_updated": datetime.now().isoformat()}
    ...
    def learn_algorithm(self, algorithm_name: str, date: Optional[datetime] = None):
        ...
        print(
            f"  Next review: {self.data['algorithms'][algorithm_name]['next_review']}"
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "average_mastery": (
                sum(a["mastery_level"] for a in algorithms.values()) / total
                if total > 0
                else 0
            ),
        }
...
def main():
    ...
    if command == "learn" and len(sys.argv) > 2:
        ...
        for algo in due:
            print(
                f"  - {algo['name']} (Level {algo['review_level'] + 1}, {algo['days_overdue']} days overdue)"
            )
        ...
        print("Usage:")
        print(
            "  python spaced_repetition_tracker.py                    # Show daily plan"
        )
        print(
            "  python spaced_repetition_tracker.py learn <algorithm>  # Mark as learned"
        )
        print(
            "  python spaced_repetition_tracker.py review <algorithm> [performance]  # Complete review"
        )
        print(
            "  python spaced_repetition_tracker.py due                # Show due reviews"
        )
        print(
            "  python spaced_repetition_tracker.py stats              # Show statistics"
        )
        print(
            "  python spaced_repetition_tracker.py plan               # Show daily plan"
        )
