...
def populate_test_file(test_path: Path, algorithm_path: Path, algorithm_name: str) -> bool:
    ...
    try:
        ...
        # Find the test class
        class_match = re.search(r'(class\s+Test\w+.*?:.*?)(def\s+test_\w+.*?:.*?pass)', content, re.DOTALL)
        if not class_match:
            return False
