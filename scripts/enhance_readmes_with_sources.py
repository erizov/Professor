...
def get_algorithm_specific_info(algorithm_name: str, category: str) -> Dict:
    ...
    algorithm_info = {
        ...
        'quick_sort': {
            ...
            'variants': ['Dual-pivot quick sort', '3-way quick sort', 'Randomized quick sort']
        },
        'merge_sort': {
            ...
            'variants': ['Bottom-up merge sort', 'Natural merge sort', 'In-place merge sort']
        },
        'binary_search': {
            ...
            'variants': ['Interpolation search', 'Exponential search', 'Ternary search']
        },
        'bfs': {
...
def enhance_readme_with_sources(readme_path: Path) -> bool:
    ...
    if external_info.get('wikipedia') and '## Introduction' in content:
        ...
        if intro_match and len(intro_match.group(2).strip()) < 200:  # Only if intro is short
            ...
            if wiki_data.get('extract'):
                ...
                content = content[:intro_match.start()] + intro_match.group(1) + enhanced_intro + content[intro_match.end():]
                changes_made = True
    ...
    if external_info.get('applications'):
        if '## Real-World Applications' in content:
            ...
            apps_section = re.search(r'(## Real-World Applications\s*\n\n)(.*?)(?=\n\n##|\Z)', content, re.DOTALL)
            if apps_section:
                existing_apps = apps_section.group(2)
                new_apps = '\n'.join([f"- **{app}**" for app in external_info['applications']])
                enhanced_apps = existing_apps + '\n\n' + new_apps
                content = content[:apps_section.start()] + apps_section.group(1) + enhanced_apps + content[apps_section.end():]
                changes_made = True
        else:
    ...
    if external_info.get('variants') and '## Algorithm Variants' not in content:
        new_sections['## Algorithm Variants'] = (
            ...
            '\n'.join([f"- **{variant}**: [Description]" for variant in external_info['variants']])
        )
        changes_made = True
    ...
    if new_sections:
        ...
        content = content[:insertion_point].rstrip() + '\n\n' + new_sections_text + '\n\n' + content[insertion_point:].lstrip()
        changes_made = True
