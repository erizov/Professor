...
def benchmark_algorithm(
    algorithm_path: Path, input_sizes: List[int], algorithm_name: str
) -> List[Dict]:
    """
...
def store_benchmark_results(results: List[Dict]):
    ...
    for result in results:
        # Get algorithm ID
        cursor.execute(
            "SELECT id FROM algorithms WHERE name = ?", (result["algorithm_name"],)
        )
        row = cursor.fetchone()
...
def benchmark_all_algorithms(
    input_sizes: List[int] = [100, 500, 1000, 5000], limit: int = None
):
    """
