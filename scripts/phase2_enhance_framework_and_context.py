...
def generate_generic_framework_example(
    ...
    if "sort" in algo_lower:
        examples[
            "java"
        ] = f"""// Java Collections.sort() uses optimized sorting
import java.util.*;
...
public class {algorithm_name.replace('_', '')}Example {{
    public static void main(String[] args) {{
        ...
        examples[
            "spring"
        ] = f"""// Spring Framework - Sorting in Spring Data
@Service
public class OrderService {{
    @Autowired
    private OrderRepository orderRepository;
    ...
    # Searching algorithms
        ...
        examples[
            "java"
        ] = f"""// Java Collections.binarySearch()
import java.util.*;
...
public class {algorithm_name.replace('_', '')}Example {{
    public static void main(String[] args) {{
        ...
        examples[
            "spring"
        ] = f"""// Spring Framework - Search in Spring Data
@Service
public class ProductService {{
    @Autowired
    ...
    # Design patterns
        ...
        examples[
            "spring"
        ] = f"""// Spring Framework - {algorithm_name.replace('_', ' ').title()} Pattern
@Component
public class {algorithm_name.replace('_', '')}Service {{
    ...
    public void execute() {{
        ...
        examples[
            "dotnet"
        ] = f"""// .NET Core - {algorithm_name.replace('_', ' ').title()} Pattern
public class {algorithm_name.replace('_', '')}Service
{{
    ...
    # Graph algorithms
        ...
        algo in algo_lower
        for algo in ["bfs", "dfs", "dijkstra", "bellman", "floyd", "graph"]
    ):
        examples[
            "java"
        ] = f"""// Java - {algorithm_name.replace('_', ' ').title()} Implementation
import java.util.*;
...
public class {algorithm_name.replace('_', '')}Example {{
    ...
    # Database-related
        ...
        db in algo_lower
        for db in ["sql", "database", "query", "join", "index", "transaction"]
    ):
        examples[
            "sql"
        ] = f"""-- SQL - {algorithm_name.replace('_', ' ').title()}
...
def enhance_framework_examples(content: str, algorithm_name: str) -> Tuple[str, bool]:
    ...
    if not examples:
        ...
        if "sort" in content.lower() or "sort" in algorithm_name.lower():
            ...
            category = "searching"
        elif "pattern" in content.lower() or any(
            p in algorithm_name.lower() for p in ["singleton", "factory", "observer"]
        ):
            category = "pattern"
    ...
    if new_section:
        ...
        content = (
            content[: match.start(2)] + new_section.strip() + content[match.end(2) :]
        )
        changed = True
...
def generate_generic_real_world_applications(
    algorithm_name: str, category: str
) -> List[str]:
    ...
    # Dynamic programming
        ...
        dp in algo_lower
        for dp in ["knapsack", "edit_distance", "longest", "fibonacci", "dynamic"]
    ):
        applications = [
    ...
    # String algorithms
        ...
        str_algo in algo_lower
        for str_algo in ["kmp", "rabin", "boyer", "string", "pattern"]
    ):
        applications = [
...
def enhance_real_world_applications(
    ...
    if not applications:
        ...
        if "sort" in algo_lower:
            ...
            p in algo_lower for p in ["singleton", "factory", "observer", "pattern"]
        ):
            category = "pattern"
        ...
        applications = generate_generic_real_world_applications(
