...
def determine_category(algorithm_name: str, lecture_name: str, content: str) -> str:
    ...
    # Sorting algorithms
    if any(s in algo_lower for s in ['sort', 'bubble', 'selection', 'insertion', 'merge', 'quick', 'heap', 'counting', 'radix', 'bucket']):
        return 'sorting'
    
    # Searching algorithms
    if any(s in algo_lower for s in ['search', 'binary', 'linear', 'jump', 'interpolation']):
        return 'searching'
    
    # Graph algorithms
    if any(s in algo_lower for s in ['bfs', 'dfs', 'dijkstra', 'bellman', 'floyd', 'graph', 'shortest', 'path']):
        return 'graph'
    
    # Tree algorithms
    if any(s in algo_lower for s in ['tree', 'bst', 'avl', 'trie', 'heap', 'binary_tree']):
        return 'tree'
    
    # Dynamic programming
    if any(s in algo_lower for s in ['knapsack', 'edit_distance', 'longest', 'fibonacci', 'dynamic', 'lcs']):
        return 'dynamic_programming'
    
    # String algorithms
    if any(s in algo_lower for s in ['kmp', 'rabin', 'boyer', 'string', 'pattern', 'matching']):
        return 'string'
    
    # Design patterns
    if any(s in algo_lower for s in ['singleton', 'factory', 'observer', 'strategy', 'adapter', 'decorator', 'proxy', 'command', 'iterator', 'composite', 'facade', 'template', 'chain', 'bridge', 'memento', 'state', 'visitor', 'builder', 'prototype']):
        return 'pattern'
    ...
    # Database-related
    if any(s in algo_lower or s in lecture_lower for s in ['sql', 'database', 'query', 'join', 'index', 'transaction', 'nosql']):
        return 'database'
    
    # CI/ML algorithms
    if any(s in algo_lower or s in lecture_lower for s in ['regression', 'classification', 'neural', 'network', 'clustering', 'svm', 'kmeans', 'decision_tree', 'random_forest']):
        return 'ci'
    
    # System/OS algorithms
    if any(s in algo_lower or s in lecture_lower for s in ['scheduling', 'memory', 'process', 'thread', 'concurrency', 'parallel']):
        return 'system'
...
def replace_todo_implementation(algo_file: Path, algorithm_name: str, category: str) -> bool:
    """Replace TODO implementation with actual algorithm logic."""
    try:
        content = algo_file.read_text(encoding='utf-8')
        
        # Check if already has implementation (not just TODO)
        if 'TODO' not in content or ('def ' in content and 'pass' not in content.split('def ')[1:][0] if len(content.split('def ')) > 1 else True):
            ...
            if 'TODO' in func_body:
                ...
                if func_match:
                    ...
                    if next_def < len(content):
                        ...
                        new_content += content[func_start:].split('def ')[-1] if 'def ' in content[func_start:] else ""
                    
                    # Fix: Keep main function if it exists
                    if 'def main()' in content:
                        main_match = re.search(r'(def main\(\):.*?)(?=\n\nif __name__|\Z)', content, re.DOTALL)
                        if main_match:
                            new_content = new_content.replace(impl, impl + "\n\n" + main_match.group(1))
    ...
    for i, (algo_file, algo_name, category) in enumerate(placeholders, 1):
        if replace_todo_implementation(algo_file, algo_name, category):
            ...
            if updated_count % 50 == 0:
                print(f"[PROGRESS] Processed {i}/{len(placeholders)} files, updated {updated_count}...")
